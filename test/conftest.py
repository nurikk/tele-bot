...
def db_mock(in_memory_db, event_loop):
    async def code():
        all_questions = [
            CardRequestQuestions.DESCRIPTION,
            CardRequestQuestions.REASON,
            CardRequestQuestions.RELATIONSHIP,
        ]

        for i in range(1, 10):
            user = await TelebotUsers.create(
                id=i,
                telegram_id=i,
            ...
            for x in range(1, 10):
                ...
                for language_code in ["en", "ru"]:
                    await Holidays.create(
                        ...
                        date=current_date + timedelta(days=x),
                        title=f"holiday {i} {x} {language_code}",
        ...
        await TelebotUsers.create(
            telegram_id=309511727, username="anonymass", full_name="Anonymass"
        )
...
def mock_open_ai_client():
    ...
    mock_async_openai_client.images.generate.return_value = Mock(
        data=[mock_image_response]
    )
    mock_async_openai_client.chat.completions.create.return_value = Mock(
        choices=[mock_choice]
    )
...
def settings():
    ...
    return Settings(
        _env_file=find_dotenv(".env", raise_error_if_not_found=True, usecwd=True)
    )

...
async def convert_to_sd_prompt(text: str, locale: str, async_openai_client: AsyncOpenAI) -> str:
    ...
    messages = [{
        ...
        "role": "user",
        "content": await ensure_english(text=text, locale=locale, async_openai_client=async_openai_client)
    }]
    response = await async_openai_client.chat.completions.create(
        model="gpt-4",
        messages=messages,
...
async def generate_prompt(data: dict[str, str], locale: str, async_openai_client: AsyncOpenAI) -> str:
    ...
    return await convert_to_sd_prompt(text=prompt, locale=locale, async_openai_client=async_openai_client)

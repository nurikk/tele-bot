...
async def startup(bot: Bot) -> None:
    ...
    for locale in i18n.config.settings["available_locales"]:
        await bot.set_my_description(
            description=i18n.t("bot_description", locale=locale), language_code=locale
        )
        await bot.set_my_short_description(
            short_description=i18n.t("bot_short_description", locale=locale),
...
async def set_default_commands(bot: Bot) -> None:
    commands = [card_bot_command, stop_bot_command, calendar_bot_command]
    for locale in i18n.config.settings["available_locales"]:
        await bot.set_my_commands(
            [
                BotCommand(
                    command=c.command,
                    description=i18n.t(f"bot_commands.{c.command}", locale=locale),
                )
                for c in commands
            ],
            language_code=locale,
...
if __name__ == "__main__":
    ...
    dp = Dispatcher(
        storage=RedisStorage.from_url(
            settings.redis_url, state_ttl=timedelta(days=settings.redis_ttl_days)
        ),
        ...
        image_proxy=ImageProxy(
            imgproxy_hostname=settings.imgproxy_hostname,

...
async def broadcast_handler(
    ...
    if user.is_admin:
        ...
        card_request = await db.CardRequests.get(
        ...
        for idx, recipient in enumerate(tqdm(recipients)):
            logging.info(
                f"Sending card to {recipient.full_name} {recipient.id} {recipient.telegram_id}"
            )
            try:
            ...
            try:
                if idx % 10 == 0:
                    await sent_message.edit_text(
                        f"Broadcasting cards...{idx + 1}/{total}\n{excs}"
                    )
...
def register(dp: Dispatcher):
    ...
    form_router.callback_query(
        CardActionCallback.filter(F.action == Action.ACTION_BROADCAST)
    )(broadcast_handler)

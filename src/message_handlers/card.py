...
async def debug_log(
    ...
    reply_markup = None
    if card_request.is_public:
        builder = InlineKeyboardBuilder()
        builder.button(
            text="Broadcast",
            callback_data=CardActionCallback(
                action=Action.ACTION_BROADCAST, request_id=request_id
            ).pack(),
        )
...
def generate_image_keyboad(locale: str, request_id: int) -> InlineKeyboardBuilder:
    ...
    # callback_data = CardActionCallback(
    #     action=Action.ACTION_REGENERATE, request_id=request_id
    # ).pack()
    buttons = [
...
async def send_photos(
    chat_id: int, request_id: int, image_proxy: Proxy, s3_uploader: S3Uploader, bot: Bot
):
    ...
    photos = [
        InputMediaPhoto(
            media=URLInputFile(
                url=image_proxy.get_full_image(
                    s3_uploader.get_full_s3_url(image_result.result_image)
                ),
                filename="card.png",
            )
        )
    ...
    await bot.send_media_group(
        chat_id=chat_id, media=photos, protect_content=True
    )  # reply_markup=keyboard.as_markup()


async def deliver_generated_samples_to_user(
    request_id: int,
    bot: Bot,
    user: TelebotUsers,
    locale: str,
    ...
    await bot.send_message(
        chat_id=user.telegram_id,
        text=i18n.t("share_description", locale=locale),
        reply_markup=keyboard.as_markup(),
    )
    await bot.send_message(
        chat_id=user.telegram_id, text=i18n.t("commands.card", locale=locale)
    )
...
def generate_samples_keyboard(
    samples: list[str], columns: int = 2
) -> ReplyKeyboardMarkup:
    keyboard = []
...
async def generate_answer_samples_keyboard(
    locale: str, question: CardRequestQuestions, columns: int = 2
) -> ReplyKeyboardMarkup:
...
async def generate_depictions_samples_keyboard(
    client: AsyncOpenAI, locale: str, request_id: int
) -> ReplyKeyboardMarkup:
    samples = await get_depiction_ideas(
        client=client, locale=locale, request_id=request_id
    )

    return generate_samples_keyboard(samples=samples, columns=1)


async def generate_descriptions_samples_keyboard(
    user: TelebotUsers, locale: str, samples_count: int = 5
):
    # Refactor this to make DISTINCT ON query
    answers = (
        await CardRequests.filter(
            user=user,
            answers__language_code=locale,
            answers__question=CardRequestQuestions.DESCRIPTION,
        )
        .annotate(min_created_at=Max("created_at"))
        .order_by("-min_created_at")
        .group_by("answers__answer")
        .limit(samples_count)
        .values("answers__answer")
    )
...
async def ensure_user_has_cards(
    message: types.Message, user: types.User = None
) -> bool:
...
async def command_start(message: types.Message, state: FSMContext) -> None:
    ...
    if await ensure_user_has_cards(message=message, user=message.from_user):
        request: CardRequests = await CardRequests.create(
            user=user, language_code=locale
        )
        ...
        await message.answer(
            i18n.t("card_form.reason.response", locale=locale),
            reply_markup=reason_samples_keyboard,
        )
...
async def process_reason(message: types.Message, state: FSMContext) -> None:
    ...
    await CardRequestsAnswers.create(
        request_id=request_id,
        question=CardRequestQuestions.REASON,
        answer=message.text,
    ...
    await message.answer(
        i18n.t(
            f"card_form.{CardRequestQuestions.DESCRIPTION.value}.response",
            locale=locale,
        ),
        reply_markup=answer_samples_keyboard,
    )
...
async def process_description(
    ...
    await CardRequestsAnswers.create(
        request_id=request_id,
        question=CardRequestQuestions.DESCRIPTION,
        answer=message.text,
        language_code=locale,
    )
    ...
    await message.answer(
        i18n.t("card_form.depiction.coming_up_with_ideas", locale=locale),
        reply_markup=ReplyKeyboardRemove(),
    )
    async with ChatActionSender.typing(bot=bot, chat_id=message.chat.id):
        depiction_ideas = await generate_depictions_samples_keyboard(
            locale=locale, request_id=request_id, client=async_openai_client
        )
        await message.answer(
            i18n.t(
                f"card_form.{CardRequestQuestions.DEPICTION.value}.response",
                locale=locale,
            ),
            reply_markup=depiction_ideas,
        )


async def process_depiction(
    message: types.Message,
    state: FSMContext,
    bot: Bot,
    settings: Settings,
    ...
    await CardRequestsAnswers.create(
        request_id=request_id,
        question=CardRequestQuestions.DEPICTION,
        answer=message.text,
        language_code=locale,
    )
...
def get_message_content(
    locale: str,
    reason: CardRequestsAnswers,
    full_name: str,
    photo_url: str,
    greeting_text: str,
) -> str:
    return i18n.t(
        "share_message_content_markdown",
        locale=locale,
...
async def inline_query(
    query: types.InlineQuery, bot: Bot, s3_uploader: S3Uploader, image_proxy: Proxy
) -> None:
    user = await user_from_message(telegram_user=query.from_user)
    ...
    results = []
    if request_id:
        request_qs = CardRequests.filter(id=request_id).filter(
    ...
    requests = (
        await request_qs.prefetch_related("results").order_by("-created_at").limit(10)
    )
    reply_markup = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text=i18n.t(
                        "generate_your_own", locale=query.from_user.language_code
                    ),
                    url=link,
                )
            ]
    ...
    for request in requests:
        reason = await CardRequestsAnswers.filter(
            request_id=request.id, question=CardRequestQuestions.REASON
        ).first()
        if reason:
            for result in request.results:
                photo_url = image_proxy.get_full_image(
                    s3_uploader.get_full_s3_url(result.result_image)
                )
                thumbnail_url = image_proxy.get_thumbnail(
                    s3_uploader.get_full_s3_url(result.result_image),
                ...
                results.append(
                    types.InlineQueryResultArticle(
                        ...
                        input_message_content=types.InputTextMessageContent(
                            message_text=get_message_content(
                                locale=query.from_user.language_code,
                                reason=reason,
...
async def chosen_inline_result_handler(chosen_inline_result: types.ChosenInlineResult):
    ...
    if request_id:
        ...
        await db.CardRequests.filter(id=request_id).update(
...
def register(dp: Dispatcher):
    ...
    form_router.callback_query(
        CardActionCallback.filter(F.action == Action.ACTION_REGENERATE)
    )(regenerate)

...
async def handle_referral(user_id: int, bot: Bot = None, cards_per_user: int = 5):
    ...
    await TelebotUsers.filter(id=user_id).update(
        remaining_cards=F_SQL("remaining_cards") + cards_per_user
    )
    if referee.remaining_cards <= 0 and bot:
        await bot.send_message(
            chat_id=referee.telegram_id,
            text=i18n.t("referral_bonus_granted", locale="en"),
        )
...
async def user_from_message(
    ...
    user_props = {
        ...
        "referred_by": await TelebotUsers.filter(id=referred_by).first()
        if referred_by
        else None,
    }
    (user, is_new) = await TelebotUsers.get_or_create(
        telegram_id=telegram_user.id, defaults=user_props
    )
    if is_new and referred_by:
        logging.info(
            f"Registered new referral user {user.full_name}(@{user.username}) new user, referred by {referred_by}"
        )
...
class TelebotUsers(Model):
    ...
    referred_by = fields.ForeignKeyField(
        "models.TelebotUsers", related_name="referrals", null=True, index=True
    )
...
class CardRequests(Model):
    ...
    user = fields.ForeignKeyField(
        "models.TelebotUsers", related_name="requests", index=True
    )
...
class CardResult(Model):
    ...
    request = fields.ForeignKeyField(
        "models.CardRequests", related_name="results", index=True
    )
    created_at: datetime.datetime = fields.DatetimeField(auto_now_add=True)
...
class CardShareEvent(Model):
    id: int = fields.IntField(pk=True)
    request = fields.ForeignKeyField(
        "models.CardRequests", related_name="shares", index=True
    )
    user = fields.ForeignKeyField(
        "models.TelebotUsers", related_name="shares", index=True
    )
    created_at: datetime.datetime = fields.DatetimeField(auto_now_add=True)
...
class CardRequestsAnswers(Model):
    ...
    created_at: datetime.datetime = fields.DatetimeField(auto_now_add=True)
    request = fields.ForeignKeyField(
        "models.CardRequests", related_name="answers", index=True
    )
...
TORTOISE_ORM = {
    "connections": {
        "default": os.environ.get("DB_URL", "postgres://localhost:5432/telebot2")
    },
    "apps": {
        "models": {

...
async def parse_country(country: str, country_code: str, year: int = 2024):
    ...
    for event in tqdm(calendar.walk('VEVENT')):
        ...
        url = event.get('url')
        if country_code == 'en':
            title = await ensure_english(text=title, locale='ru', async_openai_client=async_openai_client)
            description = await ensure_english(text=description, locale='ru', async_openai_client=async_openai_client)
        holidays.append({
